...
public class RedBlacktreeFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View rootview = inflater.inflate(R.layout.frag_red_black_tree,container,false);
        ArrayList<Item> items = new ArrayList<Item>();
        items.add(new Item("O(log(n)","O(log(n)","O(log(n)","O(log(n)","O(log(n)","O(log(n)","O(log(n)","O(log(n)"));
        ItemAdapter adapter = new ItemAdapter(getActivity(),items);
        ListView arraybtree = (ListView)rootview.findViewById(R.id.list_red_black_tree);
